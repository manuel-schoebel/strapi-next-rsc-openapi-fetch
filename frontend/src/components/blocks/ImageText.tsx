...
export async function ImageText({
  ...
  button,
  text,
}: components["schemas"]["ContentImageTextComponent"]): Promise<JSX.Element> {
  const gridCols = textColumns + imageColumns;
  const headlineVariant = headline?.variant || "h2";
  const headlineText = headline?.text || "";

  const html = await formatMarkdown(text || "");
      ...
      >
        ...
        >
          <ContentBlock
            ...
            headlineVariant={headline?.variant || "h2"}
            textAlign={textAlign}
            html={html || ""}
            textColor={``}
          />
